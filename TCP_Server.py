...
if not os.path.isdir(config["unknow_face_save_dir"]["unknow_face"]):
    ...
    print('create dir: ' + config["unknow_face_save_dir"]["unknow_face"])
if not os.path.isdir(config["unknow_face_save_dir"]["unknow_face_raw"]):
    os.makedirs(config["unknow_face_save_dir"]["unknow_face_raw"])
    print('create dir: ' + config["unknow_face_save_dir"]["unknow_face_raw"])
if not os.path.isdir(config["unknow_face_save_dir"]["unknow_face_env"]):
    os.makedirs(config["unknow_face_save_dir"]["unknow_face_env"])
    print('create dir: ' + config["unknow_face_save_dir"]["unknow_face_env"])
if not os.path.isdir(config["unknow_face_log_dir"]):
    os.makedirs(config["unknow_face_log_dir"])
    print('create dir: ' + config["unknow_face_log_dir"])

if config['facedetectROI'] == 'mtcnn':
    print('-----------------------------------------------------------------')
...
print('-----------------------------------------------------------------')
RESIZE_FINAL = 227
AGE_LIST = config['age_gender_model']['age_list']
tf.reset_default_graph()
sess_age = tf.Session()
with sess_age.as_default():
    model_fn = select_model(config['age_gender_model']['model_age_select'])    
    images_age = tf.placeholder(tf.float32, [None, RESIZE_FINAL, RESIZE_FINAL, 3])
    nlabels = len(AGE_LIST)
    logits = model_fn(nlabels, images_age, 1, False)
    init = tf.global_variables_initializer()
    checkpoint_path = config['age_gender_model']['model_age_dir']
    model_checkpoint_path, global_step = get_checkpoint(checkpoint_path, None, 'checkpoint')
    saver = tf.train.Saver()
    print(checkpoint_path + model_checkpoint_path.split('/')[-1], global_step)
    saver.restore(sess_age, checkpoint_path + model_checkpoint_path.split('/')[-1])
    softmax_age_output = tf.nn.softmax(logits)
...
GENDER_LIST =['M','F']
tf.reset_default_graph()
sess_gender = tf.Session()
with sess_gender.as_default():
    model_fn = select_model(config['age_gender_model']['model_gender_select'])    
    images_gender = tf.placeholder(tf.float32, [None, RESIZE_FINAL, RESIZE_FINAL, 3])
    nlabels = len(GENDER_LIST)
    logits = model_fn(nlabels, images_gender, 1, False)
    init = tf.global_variables_initializer()
    checkpoint_path = config['age_gender_model']['model_gender_dir']
    model_checkpoint_path, global_step = get_checkpoint(checkpoint_path, None, 'checkpoint')
    saver = tf.train.Saver()
    print(checkpoint_path + model_checkpoint_path.split('/')[-1], global_step)
    saver.restore(sess_gender, checkpoint_path + model_checkpoint_path.split('/')[-1])
    softmax_gender_output = tf.nn.softmax(logits)
...
def facedetectROI_img(rawframe):
    if config['facedetectROI'] == 'mtcnn':
        try:
            ...
            rgbframe = cv2.cvtColor(drawframw, cv2.COLOR_BGR2RGB)
            bounding_boxes, _ = align.detect_face.detect_face(rgbframe, minsize, pnet, rnet, onet, threshold, factor)      
            facesl = []
            bbs = []
                ...
                for bb in bounding_boxes:
                    ...
                    for bbi in bb:
                        if bbi < 0:
                            bbi = 0
                        bbn.append(int(bbi))
                    facesl.append(drawframw[int(bbn[1]):int(bbn[3]), int(bbn[0]):int(bbn[2])])
                    bbr = [bbn[0], bbn[1], bbn[2]-bbn[0], bbn[3]-bbn[1]]
                    bbs.append(bbr)
            else:
                facesl = []
                bb = []
            ...
            if len(framel) > 0:
                ...
                for frame in framel:
                    try:
                        ...
                        batch_results = sess_age.run(softmax_age_output, feed_dict={images_age:[image_batch]})
                        best = np.argmax(batch_results)
                        output = batch_results[0]
                        best_choice_age = (AGE_LIST[best], output[best])
                        
                        batch_results = sess_gender.run(softmax_gender_output, feed_dict={images_gender:[image_batch]})
                        best = np.argmax(batch_results)
                        output = batch_results[0]
                        best_choice_gender = (GENDER_LIST[best], output[best])
                 
                        thres = float(config["threshold"])                        
                        if min(ddl) < thres:
                            print( 'dd: ' + str(min(ddl)))
                            _string = Recommand_auto.final_output(all_probability[cust_id])
                            msg = 'id: ' + fn + ':' + str(round(min(ddl), 4)) + '@' + str(best_choice_age) + '@' + str(best_choice_gender) + ' | ' + _string + '_' + str(bb[num][0]) + ',' + str(bb[num][1]) + ',' + str(bb[num][2]) + ',' + str(bb[num][3])
                        else:
                            new_string = Recommand_auto.final_output(new_all_probability[cust_id])                            
                            msg = 'id: ' + 'unknow' + ':' + str(round(min(ddl), 4)) + '@' + str(best_choice_age) + '@' + str(best_choice_gender) + ' | ' + new_string + '_' + str(bb[num][0]) + ',' + str(bb[num][1]) + ',' + str(bb[num][2]) + ',' + str(bb[num][3])
